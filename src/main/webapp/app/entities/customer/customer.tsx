  ...
  return (
    <div>
      <h2 id="customer-heading">
        <Translate contentKey="jhipsterSampleApplicationApp.customer.home.title">Customers</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="jhipsterSampleApplicationApp.customer.home.createLabel">Create new Customer</Translate>
        </Link>
      </h2>
        ...
        {customerList && customerList.length > 0 ? (
            ...
            <tbody>
              {customerList.map((customer, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{customer.name}</td>
                  <td>{customer.piva}</td>
                  <td>{customer.codFisc}</td>
                  <td>{customer.location ? <Link to={`location/${customer.location.id}`}>{customer.location.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterSampleApplicationApp.customer.home.notFound">No Customers found</Translate>
            </div>
          )
        )}
      </div>
